...
## F110 — pool imbalance measurement as G3 preparation

G3 is blocked on a counterparty — the repo cannot invent the inventory — but it can prepare for
the maker by measuring session imbalance from the pool. The paper's maker sheet targets ±3.79%
net-Long imbalance at 1× turnover (Table 30), which rounds to a 4% threshold. A pure function
`computePoolHealth(longReserve, shortReserve)` in the calibrator's domain returns the long/short
percentages and the absolute imbalance, and the web's session detail page now shows them with a
