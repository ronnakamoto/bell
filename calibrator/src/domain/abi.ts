/**
 * The ABI fragments the broadcast surface encodes, and the encoding itself.
 *
 * Every method an intent step can name, plus the read getters the broadcast use case needs to
 * resolve its targets (`collateral()`, `longClaim()`, `shortClaim()` on the session, `bondToken()`
 * and `arbiter()` on the premium registry). All inputs are static types — address, uint256, uint64,
 * bytes32, bool — so the encoding is selector plus one 32-byte word per argument, with no dynamic
 * section. That is the whole of the ABI surface the participant surface touches, and it is kept
 * here so the encoding is one table plus one function rather than a dependency on an ABI library.
 *
 ...
 * Keccak is a port (`domain/ports.js`), injected like everywhere else in the tree — this module is
 * hermetic and the adapter supplies `@noble/hashes`.
 *
 * The module lives in the calibrator's domain — the shared core the web already imports — because
 * the publisher's commit broadcast (CLI) encodes the same static types, and a second encoder in
 * the web would be the same drift risk as a second copy of the constants.
 */

import { DomainError } from './models.js';
import { type Keccak } from './ports.js';
...
export const ABI_FRAGMENTS: Readonly<Record<string, AbiFragment>> = {
  ...
  commit: {
    signature: 'commit(bytes32,uint64,uint256,uint256,bytes32)',
    inputs: ['bytes32', 'uint64', 'uint256', 'uint256', 'bytes32'],
  },
...
export function fragmentOf(method: string): AbiFragment {
  ...
  if (fragment === undefined) {
    throw new DomainError(`no ABI fragment for method ${method}`);
  }
  return fragment;
}
...
/** The calldata for one call: selector then one word per argument. */
export function encodeCalldata(
  method: string,
  args: readonly (bigint | string)[],
  keccak: Keccak,
): string {
  const fragment = fragmentOf(method);
  if (fragment.inputs.length !== args.length) {
    throw new DomainError(
      `${method} takes ${String(fragment.inputs.length)} arguments, got ${String(args.length)}`,
    );
  }
  const words = fragment.inputs.map((type, index) => encodeWord(type, argAt(args, index)));
...
function argAt(args: readonly (bigint | string)[], index: number): bigint | string {
  ...
  /* v8 ignore next 2 */
  if (value === undefined) {
    throw new DomainError(`missing argument ${String(index)}`);
  }
  return value;
}
...
function requireHex(value: bigint | string, digits: number, label: string): string {
  if (typeof value !== 'string' || !HEX_PREFIX.test(value) || value.length !== 2 + digits) {
    throw new DomainError(`${label} must be 0x followed by ${String(digits)} hex characters`);
  }
...
function requireUint(value: bigint | string, bits: number, label: string): bigint {
  if (typeof value === 'string') {
    throw new DomainError(`${label} must be a bigint`);
  }
  if (value < 0n || value >= 1n << BigInt(bits)) {
    throw new DomainError(`${label} must fit in ${String(bits)} bits`);
  }
  return value;
}
...
function requireBool(value: bigint | string): string {
  if (typeof value === 'string') {
    throw new DomainError('bool must be a bigint');
  }
  if (value === 0n) return '00';
  if (value === 1n) return '01';
  throw new DomainError('bool must be 0 or 1');
}
