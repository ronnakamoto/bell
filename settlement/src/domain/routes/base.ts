 ...
 * significant digits; CPython's `decimal` default context is 28. `adjustedGapWad` divides an 18-digit
 * gross return by the multiplier ratio and then *truncates*, and at 20 significant digits an 18-digit
 * integer quotient has only **two** fractional places left — so a true fraction at or above 0.995
 * rounds up in the second place, carries into the integer part, and truncates to one wei too many.
 *
 * Measured rather than argued, against a **60-digit** reference so that "the true value" is not itself
 * a 20-digit rounding, and measured on an *ordinary* input rather than a contrived one: for
 * `gapWad = -0.02 * WAD` and `multiplierNow = 1.000318` — a 0.03% drift, which is what a small
 * distribution looks like — the exact quotient is `979688459070015734.996271185762927389…`, the
 * oracle's adjusted gap is `-20311540929984266` and the unpinned port's is `-20311540929984265`.
 * That one wei becomes **15 wei of payoff** once `payoffLongWad` multiplies it by `lambda = 15`:
 * `304673113949763990` against `304673113949763975`.
 *
...
//
// `payoffLongWad`, `SettleOnPrintOptions` and `settleOnPrint` live in `settle.ts`, which imports this
// module rather than the reverse. Every route that settles on a print goes through them, and the
// corporate-action adjustment they carry is guard G8 -- not a route's choice.
