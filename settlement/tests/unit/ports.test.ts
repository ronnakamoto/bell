 ...
 *
 * Ported from `settlement/tests/unit/test_ports.py`, which has 4 tests; this file has 6. The four are
 * kept and two runtime tests are added, and the reason is the one departure worth stating.
 *
 ...
 *
 * The two runtime tests exist because the type-level check says nothing about the *runtime* contract
 * the ports carry, which is the one a caller actually depends on: `printsFor` resolves a sequence,
 * and `rowsDigest` resolves `undefined` rather than rejecting, because an unavailable input is an
 * ordinary outcome. The Python's suite could not see either, since nothing implemented the protocols.
 *
...
describe('the ports', () => {
  ...
  it('the two ports are not interchangeable', () => {
    // Each port declares exactly one method, and they are different methods. Worth asserting because
    // the two are structurally similar — one method, taking one argument, returning an
    ...
    // @ts-expect-error — a print source declares no `rowsDigest`.
